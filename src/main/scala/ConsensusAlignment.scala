...
class ConsensusAlignment(read: MappingRead) extends ArrayBuffer[MappingRead]() {
  ...
  def alignOneEnd2(groupSeq: String, read: MappingRead, readST: SuffixTree): (Int, Int, Array[Int]) = {
    val readSeq = read.seq
    val mapping = Array.fill[Int](groupSeq.length)(-1)
    val rmapping = Array.fill[Int](readSeq.length)(-1)
    val kmerPos = mutable.Map[String,ArrayBuffer[Int]]()
    for ( i <- 0 to groupSeq.length - Settings.K){
      val kmer = groupSeq.substring(i,i+Settings.K)
      kmerPos.getOrElseUpdate(kmer,ArrayBuffer[Int]()).append(i)
    }
    val offset = mutable.Map[Int,Double]().withDefaultValue(0.0)
    for ( i <- 0 to readSeq.length - Settings.K) {
      val kmer = readSeq.substring(i,i+Settings.K)
      val matches = kmerPos.getOrElse(kmer,null)
      if (matches!=null && matches.size == 1){
        for ( j <- matches) offset(i-j) += 1.0//matches.length
      }
    }
    ...
    val bestMatches = ArrayBuffer[(Int,Int,Double)]()
    for ( i <- 0 to readSeq.length - Settings.K) {
      val kmer = readSeq.substring(i,i+Settings.K)
      val matches = kmerPos.getOrElse(kmer,null)
      if (matches!=null){ // && matches.length <= 10
        val candidates = matches.filter(j=> 0 <= i-j-core._1 && i-j-core._1<=Settings.MAX_INDEL)
        if (candidates.nonEmpty && candidates.size==1){
          val best = candidates.maxBy(j=>offset(i-j))
          bestMatches.append((best,i,offset(i-best)))
        }
      }
    }
    for ( (gPos,rPos,support) <- bestMatches.sortBy(-_._3)) {
      for ( i <- 0 until Settings.K if mapping(gPos+i)== -1 && rmapping(rPos+i)== -1) {
        mapping(gPos+i) = rPos+i
        rmapping(rPos+i) = gPos+i
      }
    }

    var matchCount = 0
    var spanCount = 0
    var lastG = mapping.indices.find(x => mapping(x) != -1).get
    var lastR = mapping(lastG)
    val unmatchedReadHead = Math.min(lastR, lastG + 2)
    val unmatchedGroupHead = Math.min(lastG, lastR + 2)
    ...
    for ( gs <- mapping.indices if mapping(gs) != -1) {
      val rs = mapping(gs)
      if (lastG<gs && lastR<rs){
      ...
      matchCount += 1
      spanCount += Math.max(gs - lastG, rs - lastR) + 1
      lastG = gs+1
      lastR = rs+1
    }
